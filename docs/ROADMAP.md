...
## Phase 2.7 — Fix the Emoji Selector icon, CLI tools actually launch, uniform PNG icons (done)

Three requests in one session: a specific icon looked wrong, CLI tools "don't open" when clicked (the Phase 1.2 known gap, above), and the icon set should be one uniform format.

- [x] **Emoji Selector icon fixed** — root cause: its `.desktop` file's `Icon=preferences-desktop-emoticons` lives under the theme's `preferences/` directory, but `find_icon_file()` only ever searched `apps/`. Added a whole-theme fallback search when the `apps/`-scoped one finds nothing. Fixed 5 other previously-missing icons on this machine as a side effect (68/72 → 73/73 resolved).
- [x] **CLI tools now actually launch**, instead of being greyed out or removed (the user offered several options; decided a real fix was better than working around the limitation). `launch_app` gained a `cli` parameter; when true on Linux, `linux_terminal_spawn()` tries `konsole`/`gnome-terminal`/`xfce4-terminal`/`alacritty`/`kitty`/`xterm` in order and runs the tool inside whichever is found, wrapped so the window stays open after a quick command finishes. **Verified working** — manually confirmed via Konsole running `tree`, screenshotted with real output and the "press any key to close" prompt showing. Windows gets the analogous `cmd /k`, unverified like the rest of Windows support.
- [x] **Real bug found while wiring this up**: `merge_system_scan()` was unconditionally setting `cli: false` on every system-scanned entry, silently overwriting the dataset's correct `cli: true` for anything that *also* has a local `.desktop` file — `btop` is explicitly in `CLI_ID_OVERRIDES` but was still showing as a broken "GUI app" because of this. Fixed by OR-ing the scan's own signal with any pre-existing entry's `cli` flag, and by teaching `scan_system_apps.py` to read `Terminal=` from the `.desktop` file itself as a first-class `cli` signal (previously not read at all).
- [x] **CLI tools grouped into a dedicated "⌨️ CLI Tools" section** instead of scattered across their nominal categories (is `tree` really a "System" app?), with a `Hide`/`Show` toggle persisted in `localStorage` (a display preference, not catalog data, so it doesn't need `overrides.json`).
- [x] **All icons converted to one uniform PNG format** via ImageMagick (`magick <svg> -background none -resize 128x128 <png>`) — was a mix of PNG (dashboard-icons, system-theme) and SVG (Iconify, hand-authored glyphs, some system-theme icons), which had already caused two real bugs (the XML-escaping bug and the icon-extension-mismatch bug in earlier phases). `default_icon()` simplified to a pure naming convention now that there's nothing to branch on.
- [x] **Vector sources archived, not discarded** — user's call: SVGs are losslessly re-editable, converting-then-deleting throws that away for no reason. `tools/icon_sources/` (outside `src/`, not shipped) now holds the original SVG for anything that was vector before rasterization: the 24 Iconify-sourced icons (recovered from git history, since the original bulk conversion had already deleted them before this was raised — no re-fetching needed), the 10 category glyphs + CLI glyph (regenerated), and going forward, `gen_category_glyphs.py`/`sync_icons.py`/`scan_system_apps.py` all archive-then-convert automatically instead of converting in place.
